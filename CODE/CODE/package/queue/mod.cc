...
static const char rcsid[] =
     "@(#) $Header: /cvsroot/nsnam/ns-2/queue/mod.cc,v 1.00 2011/11/04 11:21:00 seashadow Exp $ (LBL)";
#endif
...
static class MODClass : public TclClass {
public:
	MODClass() : TclClass("Queue/MOD") {}
	TclObject* create(int argc, const char*const* argv) {
		//printf("creating MOD Queue. argc = %d\n", argc);
		
		//mod to enable MOD to take arguments
		if (argc==5) 
			return (new MODQueue(argv[4]));
		else
			return (new MODQueue("Drop"));
	}
} class_mod;
 ...
 */
MODQueue::MODQueue(const char * trace) : link_(NULL), de_drop_(NULL), EDTrace(NULL), tchan_(0), idle_(1), idletime_(0.0)
{
	initParams();
	
	//	printf("Making trace type %s\n", trace);
	if (strlen(trace) >=20) {
		printf("trace type too long - allocate more space to traceType in mod.h and recompile\n");
		exit(0);
	}
	...
	bind("adaptive_", &edp_.adaptive);          // 1 for adaptive MOD
	bind("cautious_", &edp_.cautious);          // 1 for cautious marking
	bind("alpha_", &edp_.alpha); 	  	    // adaptive MOD param
	bind("beta_", &edp_.beta);                  // adaptive MOD param
	bind("interval_", &edp_.interval);	    // adaptive MOD param
	bind("feng_adaptive_",&edp_.feng_adaptive); // adaptive MOD variant
	...
	bind_bool("gentle_", &edp_.gentle);         // increase the packet
						    ...
						    // when ave queue
						    // exceeds maxthresh
	bind("function_", &edp_.func);		    //feipeng: function number: 0~4
	bind("warn_line_", &edp_.warn_line);	//feipeng: the warn line for buffer overflow
 ...
 * simulation, the bandwidth-dependent MOD parameters do not change.
 * This should be fixed, but it would require some extra parameters,
 * and didn't seem worth the trouble...
 */
void MODQueue::initialize_params()
{
/*
 ...
 */
void MODQueue::updateMaxP(double new_ave, double now)
{
 ...
 */
double MODQueue::estimator(int nqueued, int m, double ave, double q_w, int buf_s)
{
	double new_ave, old_ave;
	double diff, q_w_n;
	int ratio;

	/*Feipeng: dynamic adjustment of queue weight q_w, according to Refined Design of Random Early Detection Gateways*/
	if (nqueued > ave) {
		diff = nqueued - ave;
	} else {
		diff = 0;
	} 
	ratio = int(10*diff / buf_s);
	if (nqueued <= edp_.warn_line*buf_s) {
		//if queue size is below the warn line, we do nothing
		q_w_n = q_w;
	} else {
		//if queue size is above the warn line, adjust the queue weight
		switch (ratio) {
			case 0:
				q_w_n = q_w;
				break;
 ...
 */
double
MODQueue::calculate_p_new(double v_ave, double th_max, int gentle, double v_a, 
	double v_b, double v_c, double v_d, double max_p)
{
	...
	if (gentle && v_ave >= th_max) {
		...
		p = v_c * v_ave + v_d;
                ...
                // NEW: p is set to 1.0 
                p = 1.0;
		//printf("calculate_p_new=%f\n", p);
		//printOn = 1;
        } else {
		//scenario 1: concave: use a * log(x) + b
		//scenario 2: from th_min to (th_min+th_max)/2, refer to report for details
			//from (th_min+th_max)/2 to th_max, refer to report for details
		//scenario 3: from th_min to (th_min+th_max)/2, 
			//from (th_min+th_max)/2 to th_max, refer to report for details
		//scenario 4: convex, use a * exp(x) + b, refer to report for details
 ...
 */
double
MODQueue::modify_p(double p, int count, int count_bytes, int bytes, 
 ...
 */
int
MODQueue::drop_early(Packet* pkt)
{
 ...
 */
Packet*
MODQueue::pickPacketForECN(Packet* pkt)
{
...
void MODQueue::enque(Packet* pkt)
{
	 ...
	 */
	//printf("qib_=%d, q_->byteLength() = %d, q_->length()=%d, m=%d, edv_v_ave=%6.4f, edp_.q_w=%6.4f\n", qib_, q_->byteLength(), q_->length(), m, double(edv_.v_ave), double(edp_.q_w));
	edv_.v_ave = estimator(qib_ ? q_->byteLength() : q_->length(), m + 1, edv_.v_ave, edp_.q_w, qlim_);
...
int MODQueue::command(int argc, const char*const* argv)
{
	...
	if (argc == 2) {
		...
		if (strcmp(argv[1], "attach") == 0) {
			...
			if (tchan_ == 0) {
				tcl.resultf("MOD: trace: can't attach %s for writing", id);
				return (TCL_ERROR);
			}
			return (TCL_OK);
		}
		// tell MOD about link stats
		if (strcmp(argv[1], "link") == 0) {
			LinkDelay* del = (LinkDelay*)TclObject::lookup(argv[2]);
			if (del == 0) {
				tcl.resultf("MOD: no LinkDelay object %s",
					argv[2]);
				return(TCL_ERROR);
			}
			// set ptc now
...
MODQueue::trace(TracedVar* v)
{
	...
	if (((p = strstr(v->name(), "ave")) == NULL) &&
		...
		fprintf(stderr, "MOD:unknown trace var %s\n",
			v->name());
		return;
	}
	...
	if (tchan_) {
		...
		// XXX: be compatible with nsv1 MOD trace entries
...
void MODQueue::print_edv()
{
	printf("v_a: %f, v_b: %f\n", edv_.v_a, edv_.v_b);
}

void MODQueue::print_summarystats()
{
	//double now = Scheduler::instance().clock();
